...
public class KinectInputData
{
    ...
    {
        float handX = HandPosition.x * handScreenPositionMultiplier;
        float handY = HandPosition.y * handScreenPositionMultiplier;
        float handZ = HandPosition.z; //* (handScreenPositionMultiplier / 2);
    ...
    /*    Debug.Log(HandPosition.x);
        if (HandPosition.x > 5.7f)
        {
            handX = 5.7f * handScreenPositionMultiplier;
        }
*/

        return Camera.main.WorldToScreenPoint(new Vector3(handX, handY, handZ));
